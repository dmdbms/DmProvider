...
namespace Dm.filter
{
	internal class ConnPoolFilter : IFilter
	{
		private class ConnPoolFilterThreadHolder
		{
			internal static readonly ConnPoolFilter instance = new ConnPoolFilter();
		}

		private ConnPoolCache _connPoolCache = new ConnPoolCache();

		internal static ConnPoolFilter Instance => ConnPoolFilterThreadHolder.instance;

		internal int ConnPoolSize
		{
			set
			{
				_connPoolCache.Limit = value;
			}
		}
		...
		public void Open(FilterChain chain, DmConnection conn)
		{
			...
			string key = getKey(conn);
			while (true)
			{
				DmConnInstance dmConnInstance = _connPoolCache.Get(key);
				if (dmConnInstance == null)
				{
					break;
				}
				if (!conn.ConnProperty.ConnPoolCheck || CheckConnectionSurvival(dmConnInstance))
				{
					dmConnInstance.ConnProperty.ClearAutoCommit();
					dmConnInstance.Conn = conn;
				...
				dmConnInstance.Close(keep_tcp: false);
			}
			chain.Open(conn);
		}

		private string getKey(DmConnection conn)
		{
			return conn.ConnProperty.ServName + "/" + conn.ConnProperty.User;
		}

		private bool CheckConnectionSurvival(DmConnInstance connInstance)
		{
			bool result = false;
			try
			{
				...
				dmConnection.SetState(ConnectionState.Open);
				...
				result = dmDataReader.do_Read() && dmDataReader.do_GetInt32(0) == 1;
				dmDataReader.do_Close();
				dmCommand.Close();
				dmConnection.ReleaseUnmanagedResource(pooled: true);
				dmConnection.SetState(ConnectionState.Closed);
				return result;
			}
			catch (Exception ex)
			{
		...
		public void Close(FilterChain chain, DmConnection conn)
		{
			...
			if (connInst != null)
			{
				...
				if (!_connPoolCache.Put(connInst, key))
				{
					connInst.Close(keep_tcp: false);
				}
			}
			chain.Close(conn);
		}
