...
{
	internal class RWFilter2 : IFilter
	{
		private class RWFilter2ThreadHolder
		{
			internal static readonly RWFilter2 instance = new RWFilter2();
		}

		internal static RWFilter2 Instance => RWFilter2ThreadHolder.instance;
		...
		public void Open(FilterChain chain, DmConnection conn)
		{
			...
			{
				conn.ConnProperty.LoginPrimary = LoginModeFlag.onlyprimary;
				chain.Open(conn);
		...
		{
			DmCommand dmCommand = chain.CreateDbCommand(conn);
		...
		public void Prepare(FilterChain chain, DmCommand command)
		{
			...
			RWUtil2.execute(command, (execute<object>)delegate
			{
				command.RWInfo.cmdCurrent.do_Prepare();
				return null;
			}, (closeDmDataReader<object>)delegate
			{
			}, (executeByOther<object>)delegate(DmCommand otherCmd)
			{
				otherCmd.do_Prepare();
				return null;
			});
		...
		public DmParameter CreateDbParameter(FilterChain chain, DmCommand command)
		{
			...
			{
				return chain.CreateDbParameter(command.RWInfo.cmdCurrent);
			}
